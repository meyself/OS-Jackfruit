 ...
 *
 * A spinlock (not a mutex) protects the list because the timer callback
 * runs in softirq context and cannot sleep.  Process-context ioctl paths
 * use spin_lock_bh() to disable softirqs on the local CPU.
 */
 ...
 *
 * Uses list_for_each_entry_safe() so entries can be removed during
 ...
 *   - MONITOR_REGISTER   register a PID with soft + hard limits
 *   - MONITOR_UNREGISTER remove a PID when no longer needed
 * --------------------------------------------------------------- */
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        /* Allocate before acquiring the spinlock (kmalloc may sleep with
           GFP_KERNEL; spinlock context must not sleep). */
...
static void __exit monitor_exit(void)
{
    ...
    /* Free every remaining list entry */
    spin_lock_bh(&monitored_lock);
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        list_del(&entry->list);
        kfree(entry);
